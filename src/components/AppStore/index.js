...
class AppStore extends Component {
  ...
  getFilteredApps = serachResults => {
    const {activeTabId} = this.state

    const filteredApps = serachResults.filter(
  ...
  render() {
    ...
    const serachResults = appDetailsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    // method 2 for getting results
    const filteredApps = this.getFilteredApps(serachResults)
